...
class OdeskBot(object):
    ...
    def getTest(self):
        ...
        if not h1 or h1.getText().strip() != testHeader: # Check if we are already in the test launch page
            ...
            self.testObject.questionAnswers[dummyData[1]] = [ dummyData[2], dummyData[3][1], dummyData[3][0]]
    ...
    def getQuestion(self):
        ...
        if not testForm or not testForm.has_key("action"):
            return( [ -1,  None, {} ] ) # Could not find the form in HTML
    ...
    def _addToDB(self, qInfo):
        question = qInfo[1]
        answer = qInfo[3][1]
        if  re.search(re.compile(r",", re.MULTILINE |  re.DOTALL), answer):
            answer = answer.split(",")
        testId = qInfo[3][0]
...
class GUIWrapper(object):
    ...
    def __init__(self):
        ...
        self.noticeBox = wx.StaticText(self.panel, wx.ID_ANY, "Loading...", pos=(110, 10)) # A place to write out any warnings or status messages.
        self.testListLabel = wx.StaticText(self.panel, wx.ID_ANY, "Select Any Test: ", pos=(20, 30))
        self.testListBox = wx.ListBox(self.panel, wx.ID_ANY, choices=['Select One', ], pos=(110, 30))
        self.reanswerCheckBox = wx.CheckBox(self.panel, -1, "Re-answer: ", pos=(220, 30))
        self.takeTestButton = wx.Button(self.panel, wx.ID_ANY, "Start Test", pos=(340, 30))
...
class odeskThread(Thread):
    def __init__(self, guiObject, threadId, threadName, threadCounter):
    ...
    def run(self):
        ...
        #loggedin = self.guiObject.login("supmit", "spmprx13")
...
if __name__ == "__main__":
    gui = GUIWrapper()
    threadObject = odeskThread(gui, 1, "backgroundOps", 1)
    threadObject.start()
    gui.app.MainLoop()
