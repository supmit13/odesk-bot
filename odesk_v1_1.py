...
class OdeskBot(object):
    ...
    def login(self, username=None, password=None, loginUrl = r"https://www.odesk.com/login"):
        ...
        try:
            ...
            print __file__.__str__() + ": The connection is not working right now...1 Will wait for a little while before trying again. - %s\n"%(sys.exc_info()[1].__str__())
            return None
        ...
        if not self.requestUrl:
            ...
            return None
        self.requestUrl = self.requestUrl.strip()
        self.pageRequest = urllib2.Request(self.requestUrl, None, self.httpHeaders)
        try:
            self.pageResponse = self.no_redirect_opener.open(self.pageRequest)
            self.sessionCookies = self.__class__._getCookieFromResponse(self.pageResponse)
        except:
            print __file__.__str__() + ": The connection is not working right now...2 Will wait for a little while before trying again. - %s\n"%(sys.exc_info()[1].__str__())
            return None
        responseHeaders = self.pageResponse.info()
        if responseHeaders.has_key('Location'):
            self.httpHeaders['Referer'] = self.requestUrl
        ...
        try:
            ...
            print __file__.__str__() + ": The connection is not working right now...3 Will wait for a little while before trying again. - %s\n"%(sys.exc_info()[1].__str__())
            return None
        responseHeaders = self.pageResponse.info()
        self.requestUrl = self.requestUrl.strip()
        if responseHeaders.has_key('Location'):
        ...
        try:
            ...
            print __file__.__str__() + ": The connection is not working right now...4 Will wait for a little while before trying again. - %s\n"%(sys.exc_info()[1].__str__())
            return None
        ...
        while nextUrl:
            ...
            try:
                ...
                if responseHeaders.has_key('Location'):
                    ...
                    print __file__.__str__() + ": The connection is not working right now...5 Will wait for a little while before trying again. - %s\n"%(sys.exc_info()[1].__str__())
                    return None
        self.currentPageContent = self.__class__._decodeGzippedContent(self.getPageContent())
        if not self.currentPageContent:
            print "Could not access the website content of: '" + self.requestUrl + "'...\n\n-------------------------\n\n"
    ...
    def getTest(self):
        ...
        f = open("teststartpagecontent.html", "w")
        f.write(testStartPageContent)
        f.close()
    ...
    def getNextQuestion(self, questionInfo):
        soup = BeautifulSoup(self.currentPageContent)
        h1 = soup.find("h1")
        if h1 and h1.getText() == "Test complete!":
            return(-1)
